...
--- Returns the media table or nil. These keys may or may not be available:
-- Handler, Arguments, StartAt, StartTime, Resumable, LowFramerate, StopTime.
-- These fields are provided by metadata functions and may or may not be
-- available: Provider, URL, Title, Description, Length, Tags, DatePublished,
-- DateModified, Submitter, SubmitterURL, SubmitterAvatar, NumFaves, NumViews,
-- NumComments, RatingNorm, NumRatings, Thumbnail, Width, Height, IsLive,
-- ViewerCount.
-- @return Media table
function ENT:GetMedia()
    return self.Media 
end
...
function ENT:UpdateMetadata(data)    
    if self.Media then
        ...
        if self.Media.Length ~= nil then
            ...
            if self.Media.StopTime > RealTime() then
	            timer.Create(timerID,
	                         self.Media.StopTime - RealTime(), 1,
...
function ENT:WireInputProtectionTriggered()
    return GetConVar("playx_wire_input_delay"):GetFloat() > 0 and 
        ...
        (RealTime() - self.LastBeginTime) < GetConVar("playx_wire_input_delay"):GetFloat()
end
