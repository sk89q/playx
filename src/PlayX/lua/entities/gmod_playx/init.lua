...
function ENT:OpenMedia(provider, uri, start, forceLowFramerate, useJW)
    ...
    -- Hook
    local r, err = self:MediaOpen(provider, uri, start, forceLowFramerate, useJW, result)
    if r == false then
        return false, err
    ...
    if result.QueryMetadata then
        ...
        function(err)
            if ValidEntity(self) then self.MetadataError(err) end
        end)
    end
...
function ENT:MediaOpen(provider, uri, start, forceLowFramerate, useJW, result)
    return hook.Call("PlayXMediaOpen", GAMEMODE, self, 
                     provider, uri, start, forceLowFramerate, useJW, result)
end
